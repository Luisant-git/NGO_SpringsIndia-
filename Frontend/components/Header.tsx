...
const navLinks = [
  ...
  { 
    ...
    dropdown: [
      { name: 'Project SheWins', path: '/project-shewins' },
                ...
                <div key={link.name}>
                  ...
                  {openDropdown === link.name && (
                    <div className="pl-6 pt-1 pb-2">
                      {link.dropdown.map(item => (
                        <Link key={item.name} to={item.path} onClick={closeMobileMenu} className="block px-3 py-2 rounded-md text-lg font-medium text-gray-600 hover:bg-teal-100 hover:text-orange-500 uppercase">
                          {item.name}
                        </Link>
                      ))}
                    </div>
